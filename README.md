...
- Governance policy is intentionally simple (single assigned reviewer policy with explicit demo-grade RBAC enforcement).
...
- reviewer decisions restricted to assigned cases; contributor recall/resubmit restricted to own requests
- unified admin intake visibility for all submitted requests (clean, flagged, recalled, under review, approved/rejected)
- contributor structured submission fields for `optional_note`, `scenario_id`, `revision_reference`, and guided `property/domain/range`
...
### Request lifecycle (demo policy)
The admin intake table exposes a lightweight status model derived from registry + governance state:
- `submitted`
- `checked`
- `ready_for_admin_triage`
