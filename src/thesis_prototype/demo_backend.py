...
class DemoBackend:
    ...
    def __init__(self) -> None:
        ...
        self._recalled_change_ids: set[str] = set()
    ...
    def detect_conflicts(self, change_id: str, mode: str = "all") -> List[Dict[str, Any]]:
        if change_id in self._recalled_change_ids:
            raise ValueError("cannot run conflict detection on a recalled change")
    ...
    def validate_change(self, change_id: str) -> List[Dict[str, Any]]:
        if change_id in self._recalled_change_ids:
            raise ValueError("cannot validate a recalled change")
        change = self.registry.get_change(change_id)
    ...
    def get_request_overview_rows(self, actor_user_id: str) -> List[Dict[str, Any]]:
        actor = self.auth.get_user_by_id(actor_user_id)
        rows: List[Dict[str, Any]] = []
        for change in self.list_changes(actor_user_id=actor_user_id):
            issue_count = len(self.registry.get_issues_for_change(change.change_id))
            validation_results = self.registry.get_validation_results_for_change(change.change_id)
            fail_count = sum(1 for r in validation_results if r.status.value == "fail")
            case = self._find_related_review_case(change.change_id)
            assigned_reviewer = case.assigned_reviewer if case else ""
            if actor.role is Role.REVIEWER and assigned_reviewer != actor_user_id:
                continue
            status = self._derive_request_status(change.change_id)
            rows.append(
                {
                    "change_id": change.change_id,
                    "submitted_by_user_id": change.contributor_id,
                    "timestamp": change.timestamp.isoformat(),
                    "affected_entity": change.affected_entity,
                    "operation_type": change.operation_type.value,
                    "status": status,
                    "optional_note": change.optional_note or "",
                    "scenario_id": change.scenario_id or "",
                    "revision_reference": change.revision_reference or "",
                    "issue_count": issue_count,
                    "failed_validation_count": fail_count,
                    "has_review_case": bool(case),
                    "assigned_reviewer_user_id": assigned_reviewer,
                    "last_action_by_user_id": assigned_reviewer or change.contributor_id,
                    "property": change.proposed_values.get("property", ""),
                    "domain": change.proposed_values.get("domain", ""),
                    "range": change.proposed_values.get("range", ""),
                }
            )
        return rows
    ...
    def approve_review(self, review_case_id: str, reviewer_id: str, rationale: str, actor_user_id: Optional[str] = None) -> Dict[str, Any]:
        if actor_user_id:
            ...
            if actor_user_id != reviewer_id:
                raise PermissionError("reviewers can only submit decisions as themselves")
            self._require_case_access(actor_user_id, review_case_id)
    ...
    def resubmit_review_case(
        ...
        if actor_user_id:
            ...
            case = self.governance.get_review_case(review_case_id)
            if case.workflow_state.value != "revision_requested":
                raise ValueError("resubmission is allowed only for revision-requested cases")
            actor = self.auth.get_user_by_id(actor_user_id)
            if actor.role is Role.CONTRIBUTOR:
                original_change = self.registry.get_change(case.originating_change_id)
                if original_change.contributor_id != actor_user_id:
                    raise PermissionError("contributors can only resubmit their own cases")
    ...
    def recall_change(self, change_id: str, actor_user_id: str) -> Dict[str, Any]:
        ...
        change = self.registry.get_change(change_id)
        if change_id in self._recalled_change_ids:
            raise ValueError("change is already recalled")
        ...
        self._recalled_change_ids.add(change_id)
    ...
    def get_review_history(self, review_case_id: str) -> List[Dict[str, Any]]:
        # Only return history for cases visible to the actor when scoped through list_review_cases.
    ...
    def _require_case_access(self, actor_user_id: str, review_case_id: str) -> None:
        actor = self.auth.get_user_by_id(actor_user_id)
        if actor.role is Role.ADMIN:
            return
        case = self.governance.get_review_case(review_case_id)
        if actor.role is Role.REVIEWER and case.assigned_reviewer != actor_user_id:
            raise PermissionError("reviewer is not assigned to this case")

    def _find_related_review_case(self, change_id: str) -> Any:
        candidates = [
            c
            for c in self.governance.list_review_cases()
            if c.originating_change_id == change_id or c.current_change_id == change_id
        ]
        if not candidates:
            return None
        return sorted(candidates, key=lambda c: c.created_at)[-1]

    def _derive_request_status(self, change_id: str) -> str:
        if change_id in self._recalled_change_ids:
            return "recalled"
        case = self._find_related_review_case(change_id)
        if case:
            if case.workflow_state.value == "revision_requested":
                return "revision_requested"
            if case.workflow_state.value in {"awaiting_review", "in_review"}:
                return "under_review" if case.assigned_reviewer else "needs_review"
            if case.workflow_state.value == "closed":
                if case.final_decision:
                    return case.final_decision.value
                return "closed"
        issues = self.registry.get_issues_for_change(change_id)
        validations = self.registry.get_validation_results_for_change(change_id)
        has_failed_validation = any(v.status.value == "fail" for v in validations)
        if issues or has_failed_validation:
            return "ready_for_admin_triage"
        if validations:
            return "checked"
        return "submitted"
