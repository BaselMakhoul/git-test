...
def _render_admin_dashboard(st, backend: DemoBackend, current_user: Dict[str, Any]) -> None:
    ...
    intake_rows = backend.get_request_overview_rows(actor_user_id=current_user["user_id"])

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Users", len(backend.list_users(current_user["user_id"])))
    c2.metric("Changes", len(intake_rows))
    ...
    with tab_requests:
        st.markdown("**All Changes (Unified Intake)**")
        st.dataframe(normalize_records(intake_rows), use_container_width=True)
        _render_download_buttons(st, intake_rows, "admin_all_changes", "Admin Intake - All Changes")
...
def _render_reviewer_dashboard(st, backend: DemoBackend, current_user: Dict[str, Any]) -> None:
    ...
    case_rows = _review_case_rows(cases)
    pending_cases = [c for c in cases if c.get("workflow_state") in {"awaiting_review", "in_review"}]
    revision_cases = [c for c in cases if c.get("workflow_state") == "revision_requested"]

    s1, s2, s3 = st.columns(3)
    s1.metric("Assigned Cases", len(cases))
    s2.metric("Pending Action", len(pending_cases))
    s3.metric("Revision Requested", len(revision_cases))
    ...
    st.markdown("### Registry / Traceability (Assigned Cases)")
    relevant_change_ids = {c["originating_change_id"] for c in cases}
    summary = backend.get_registry_summary(actor_user_id=user_id)
    reviewer_issues = []
    reviewer_validations = []
    for change in summary["changes"]:
        if change["change_id"] in relevant_change_ids:
            reviewer_issues.extend(_issue_rows(summary["issues_by_change"].get(change["change_id"], [])))
            reviewer_validations.extend(_validation_rows(summary["validation_by_change"].get(change["change_id"], [])))
    st.markdown("**Linked Issues**")
    st.dataframe(normalize_records(reviewer_issues), use_container_width=True)
    st.markdown("**Linked Validation Results**")
    st.dataframe(normalize_records(reviewer_validations), use_container_width=True)
...
def _render_contributor_dashboard(st, backend: DemoBackend, current_user: Dict[str, Any]) -> None:
    ...
    my_changes = backend.get_request_overview_rows(actor_user_id=user_id)
    action_needed = [c for c in backend.list_review_cases(actor_user_id=user_id) if c.get("workflow_state") == "revision_requested"]

    with tab_my:
        m1, m2 = st.columns(2)
        m1.metric("My Requests", len(my_changes))
        ...
        if my_changes:
            options = {f"{c['change_id']} | {c['affected_entity']} | {c['status']}": c for c in my_changes}
    ...
    with tab_submit:
        with st.form("contributor_submit_form"):
            ...
            property_name = st.text_input("Property", value="hasSymptom")
            domain_value = st.text_input("Domain", value="Disease")
            range_value = st.text_input("Range", value="Symptom")
            optional_note = st.text_input("Optional Note", value="")
            scenario_id = st.text_input("Scenario ID", value="")
            revision_reference = st.text_input("Revision Reference", value="")
            with st.expander("Advanced: Additional proposed_values JSON (optional)"):
                extra = st.text_area("Extra JSON", value="{}")
        ...
        if submit:
            try:
                change = backend.create_change(
                    ...
                    proposed_values_extra={
                        "property": property_name,
                        "domain": domain_value,
                        "range": range_value,
                        **_parse_json_input(extra),
                    },
...
def run() -> None:
    ...
    @st.cache_resource
    def _shared_backend() -> DemoBackend:
        return DemoBackend()

    backend: DemoBackend = _shared_backend()
