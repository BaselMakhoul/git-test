...
def test_recall_and_resubmit_permissions() -> None:
    ...
    contributor_rows = backend.get_request_overview_rows(actor_user_id=contributor["user_id"])
    recalled_row = next(r for r in contributor_rows if r["change_id"] == change.change_id)
    assert recalled_row["status"] == "recalled"
...
def test_contributor_submission_fields_and_admin_visibility() -> None:
    backend = DemoBackend()
    admin = backend.login("admin", "admin123")
    contributor = backend.login("contributor", "contrib123")
    ...
    change = backend.create_change(
        ...
        operation_type="update",
        label="Heart Disease",
        description="Disease affecting the heart",
        optional_note="demo note",
        scenario_id="S-01",
        revision_reference="REV-001",
        proposed_values_extra={"property": "hasSymptom", "domain": "Disease", "range": "Symptom"},
        actor_user_id=contributor["user_id"],
    )

    contributor_rows = backend.get_request_overview_rows(actor_user_id=contributor["user_id"])
    admin_rows = backend.get_request_overview_rows(actor_user_id=admin["user_id"])
    row = next(r for r in admin_rows if r["change_id"] == change.change_id)

    assert any(r["change_id"] == change.change_id for r in contributor_rows)
    assert row["optional_note"] == "demo note"
    assert row["scenario_id"] == "S-01"
    assert row["revision_reference"] == "REV-001"
    assert row["property"] == "hasSymptom"
    assert row["domain"] == "Disease"
    assert row["range"] == "Symptom"
    assert row["status"] == "submitted"


def test_clean_and_flagged_requests_both_visible_to_admin() -> None:
    backend = DemoBackend()
    admin = backend.login("admin", "admin123")
    contributor = backend.login("contributor", "contrib123")

    clean = backend.create_change(
        contributor_id=contributor["user_id"],
        affected_entity="Disease",
        operation_type="update",
        label="Heart Disease",
    ...
    backend.validate_change(clean.change_id)
    backend.validate_change(flagged.change_id)

    rows = backend.get_request_overview_rows(actor_user_id=admin["user_id"])
    row_by_id = {r["change_id"]: r for r in rows}
    assert clean.change_id in row_by_id
    assert flagged.change_id in row_by_id
    assert row_by_id[clean.change_id]["status"] == "checked"
    assert row_by_id[flagged.change_id]["status"] == "ready_for_admin_triage"
