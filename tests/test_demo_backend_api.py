...
def test_api_path_governance_hooks_work() -> None:
    backend = DemoBackend()
    change = backend.create_change(
        contributor_id="u1",
        affected_entity="Disease",
    ...
    backend.validate_change(change.change_id)
    issue = backend.detect_conflicts(change.change_id, mode="all")
    # Open governance from failed validation artifact for deterministic test path.
    validation_results = backend.get_validation_results_for_change(change.change_id)
    failed = next(r for r in validation_results if r["status"] == "fail")
    case = backend.open_review_for_failed_validation(failed["validation_result_id"])
    assigned = backend.assign_reviewer(case["review_case_id"], "reviewer-1")
    backend.request_review_revision(case["review_case_id"], "reviewer-1", "Please fix description")
    ...
    backend.resubmit_review_case(case["review_case_id"], revised.change_id, actor_id="u1")
    history = backend.get_review_history(case["review_case_id"])

    assert assigned["assigned_reviewer"] == "reviewer-1"
